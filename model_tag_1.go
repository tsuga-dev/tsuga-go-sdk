/*
Tsuga Public API

Public HTTP API for Tsuga customers and customer-operated tools. Use these endpoints to query observability data, manage customer-owned Tsuga resources, and retrieve documentation or API-reference content. Public API requests authenticate with Bearer tokens such as operation keys. See [API reference](/documentation/api).

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package tsuga

import (
	"encoding/json"
	"fmt"
)

// checks if the Tag1 type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &Tag1{}

// Tag1 struct for Tag1
type Tag1 struct {
	// Tag key attached to the resource. Consumers can use key/value tags for filtering, ownership context, and policy checks.
	Key string `json:"key"`
	// Tag value attached to the resource for this key. Tsuga returns the value exactly as stored.
	Value                string `json:"value"`
	AdditionalProperties map[string]interface{}
}

type _Tag1 Tag1

// NewTag1 instantiates a new Tag1 object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewTag1(key string, value string) *Tag1 {
	this := Tag1{}
	this.Key = key
	this.Value = value
	return &this
}

// NewTag1WithDefaults instantiates a new Tag1 object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewTag1WithDefaults() *Tag1 {
	this := Tag1{}
	return &this
}

// GetKey returns the Key field value
func (o *Tag1) GetKey() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Key
}

// GetKeyOk returns a tuple with the Key field value
// and a boolean to check if the value has been set.
func (o *Tag1) GetKeyOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Key, true
}

// SetKey sets field value
func (o *Tag1) SetKey(v string) {
	o.Key = v
}

// GetValue returns the Value field value
func (o *Tag1) GetValue() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Value
}

// GetValueOk returns a tuple with the Value field value
// and a boolean to check if the value has been set.
func (o *Tag1) GetValueOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Value, true
}

// SetValue sets field value
func (o *Tag1) SetValue(v string) {
	o.Value = v
}

func (o Tag1) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o Tag1) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["key"] = o.Key
	toSerialize["value"] = o.Value

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

func (o *Tag1) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"key",
		"value",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err
	}

	for _, requiredProperty := range requiredProperties {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varTag1 := _Tag1{}

	err = json.Unmarshal(data, &varTag1)

	if err != nil {
		return err
	}

	*o = Tag1(varTag1)

	additionalProperties := make(map[string]interface{})

	if err = json.Unmarshal(data, &additionalProperties); err == nil {
		delete(additionalProperties, "key")
		delete(additionalProperties, "value")
		o.AdditionalProperties = additionalProperties
	}

	return err
}

type NullableTag1 struct {
	value *Tag1
	isSet bool
}

func (v NullableTag1) Get() *Tag1 {
	return v.value
}

func (v *NullableTag1) Set(val *Tag1) {
	v.value = val
	v.isSet = true
}

func (v NullableTag1) IsSet() bool {
	return v.isSet
}

func (v *NullableTag1) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableTag1(val *Tag1) *NullableTag1 {
	return &NullableTag1{value: val, isSet: true}
}

func (v NullableTag1) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableTag1) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
