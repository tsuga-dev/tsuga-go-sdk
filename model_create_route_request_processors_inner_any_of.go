/*
Tsuga Public API

HTTP API used by Tsuga customers

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package tsuga

import (
	"encoding/json"
	"time"
	"bytes"
	"fmt"
)

// checks if the CreateRouteRequestProcessorsInnerAnyOf type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &CreateRouteRequestProcessorsInnerAnyOf{}

// CreateRouteRequestProcessorsInnerAnyOf struct for CreateRouteRequestProcessorsInnerAnyOf
type CreateRouteRequestProcessorsInnerAnyOf struct {
	// Identifier of the processor
	Id string `json:"id"`
	// Display name of the processor
	Name string `json:"name"`
	Description *string `json:"description,omitempty"`
	Example *CreateRouteRequestProcessorsInnerAnyOfExample `json:"example,omitempty"`
	// List of key/value tags applied to the resource
	Tags []Tag `json:"tags,omitempty"`
	UpdatedAt *time.Time `json:"updatedAt,omitempty"`
	Type string `json:"type"`
	Params CreateRouteRequestProcessorsInnerAnyOfParams `json:"params"`
}

type _CreateRouteRequestProcessorsInnerAnyOf CreateRouteRequestProcessorsInnerAnyOf

// NewCreateRouteRequestProcessorsInnerAnyOf instantiates a new CreateRouteRequestProcessorsInnerAnyOf object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCreateRouteRequestProcessorsInnerAnyOf(id string, name string, type_ string, params CreateRouteRequestProcessorsInnerAnyOfParams) *CreateRouteRequestProcessorsInnerAnyOf {
	this := CreateRouteRequestProcessorsInnerAnyOf{}
	this.Id = id
	this.Name = name
	this.Type = type_
	this.Params = params
	return &this
}

// NewCreateRouteRequestProcessorsInnerAnyOfWithDefaults instantiates a new CreateRouteRequestProcessorsInnerAnyOf object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCreateRouteRequestProcessorsInnerAnyOfWithDefaults() *CreateRouteRequestProcessorsInnerAnyOf {
	this := CreateRouteRequestProcessorsInnerAnyOf{}
	return &this
}

// GetId returns the Id field value
func (o *CreateRouteRequestProcessorsInnerAnyOf) GetId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Id
}

// GetIdOk returns a tuple with the Id field value
// and a boolean to check if the value has been set.
func (o *CreateRouteRequestProcessorsInnerAnyOf) GetIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Id, true
}

// SetId sets field value
func (o *CreateRouteRequestProcessorsInnerAnyOf) SetId(v string) {
	o.Id = v
}

// GetName returns the Name field value
func (o *CreateRouteRequestProcessorsInnerAnyOf) GetName() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Name
}

// GetNameOk returns a tuple with the Name field value
// and a boolean to check if the value has been set.
func (o *CreateRouteRequestProcessorsInnerAnyOf) GetNameOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Name, true
}

// SetName sets field value
func (o *CreateRouteRequestProcessorsInnerAnyOf) SetName(v string) {
	o.Name = v
}

// GetDescription returns the Description field value if set, zero value otherwise.
func (o *CreateRouteRequestProcessorsInnerAnyOf) GetDescription() string {
	if o == nil || IsNil(o.Description) {
		var ret string
		return ret
	}
	return *o.Description
}

// GetDescriptionOk returns a tuple with the Description field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CreateRouteRequestProcessorsInnerAnyOf) GetDescriptionOk() (*string, bool) {
	if o == nil || IsNil(o.Description) {
		return nil, false
	}
	return o.Description, true
}

// HasDescription returns a boolean if a field has been set.
func (o *CreateRouteRequestProcessorsInnerAnyOf) HasDescription() bool {
	if o != nil && !IsNil(o.Description) {
		return true
	}

	return false
}

// SetDescription gets a reference to the given string and assigns it to the Description field.
func (o *CreateRouteRequestProcessorsInnerAnyOf) SetDescription(v string) {
	o.Description = &v
}

// GetExample returns the Example field value if set, zero value otherwise.
func (o *CreateRouteRequestProcessorsInnerAnyOf) GetExample() CreateRouteRequestProcessorsInnerAnyOfExample {
	if o == nil || IsNil(o.Example) {
		var ret CreateRouteRequestProcessorsInnerAnyOfExample
		return ret
	}
	return *o.Example
}

// GetExampleOk returns a tuple with the Example field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CreateRouteRequestProcessorsInnerAnyOf) GetExampleOk() (*CreateRouteRequestProcessorsInnerAnyOfExample, bool) {
	if o == nil || IsNil(o.Example) {
		return nil, false
	}
	return o.Example, true
}

// HasExample returns a boolean if a field has been set.
func (o *CreateRouteRequestProcessorsInnerAnyOf) HasExample() bool {
	if o != nil && !IsNil(o.Example) {
		return true
	}

	return false
}

// SetExample gets a reference to the given CreateRouteRequestProcessorsInnerAnyOfExample and assigns it to the Example field.
func (o *CreateRouteRequestProcessorsInnerAnyOf) SetExample(v CreateRouteRequestProcessorsInnerAnyOfExample) {
	o.Example = &v
}

// GetTags returns the Tags field value if set, zero value otherwise.
func (o *CreateRouteRequestProcessorsInnerAnyOf) GetTags() []Tag {
	if o == nil || IsNil(o.Tags) {
		var ret []Tag
		return ret
	}
	return o.Tags
}

// GetTagsOk returns a tuple with the Tags field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CreateRouteRequestProcessorsInnerAnyOf) GetTagsOk() ([]Tag, bool) {
	if o == nil || IsNil(o.Tags) {
		return nil, false
	}
	return o.Tags, true
}

// HasTags returns a boolean if a field has been set.
func (o *CreateRouteRequestProcessorsInnerAnyOf) HasTags() bool {
	if o != nil && !IsNil(o.Tags) {
		return true
	}

	return false
}

// SetTags gets a reference to the given []Tag and assigns it to the Tags field.
func (o *CreateRouteRequestProcessorsInnerAnyOf) SetTags(v []Tag) {
	o.Tags = v
}

// GetUpdatedAt returns the UpdatedAt field value if set, zero value otherwise.
func (o *CreateRouteRequestProcessorsInnerAnyOf) GetUpdatedAt() time.Time {
	if o == nil || IsNil(o.UpdatedAt) {
		var ret time.Time
		return ret
	}
	return *o.UpdatedAt
}

// GetUpdatedAtOk returns a tuple with the UpdatedAt field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CreateRouteRequestProcessorsInnerAnyOf) GetUpdatedAtOk() (*time.Time, bool) {
	if o == nil || IsNil(o.UpdatedAt) {
		return nil, false
	}
	return o.UpdatedAt, true
}

// HasUpdatedAt returns a boolean if a field has been set.
func (o *CreateRouteRequestProcessorsInnerAnyOf) HasUpdatedAt() bool {
	if o != nil && !IsNil(o.UpdatedAt) {
		return true
	}

	return false
}

// SetUpdatedAt gets a reference to the given time.Time and assigns it to the UpdatedAt field.
func (o *CreateRouteRequestProcessorsInnerAnyOf) SetUpdatedAt(v time.Time) {
	o.UpdatedAt = &v
}

// GetType returns the Type field value
func (o *CreateRouteRequestProcessorsInnerAnyOf) GetType() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Type
}

// GetTypeOk returns a tuple with the Type field value
// and a boolean to check if the value has been set.
func (o *CreateRouteRequestProcessorsInnerAnyOf) GetTypeOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Type, true
}

// SetType sets field value
func (o *CreateRouteRequestProcessorsInnerAnyOf) SetType(v string) {
	o.Type = v
}

// GetParams returns the Params field value
func (o *CreateRouteRequestProcessorsInnerAnyOf) GetParams() CreateRouteRequestProcessorsInnerAnyOfParams {
	if o == nil {
		var ret CreateRouteRequestProcessorsInnerAnyOfParams
		return ret
	}

	return o.Params
}

// GetParamsOk returns a tuple with the Params field value
// and a boolean to check if the value has been set.
func (o *CreateRouteRequestProcessorsInnerAnyOf) GetParamsOk() (*CreateRouteRequestProcessorsInnerAnyOfParams, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Params, true
}

// SetParams sets field value
func (o *CreateRouteRequestProcessorsInnerAnyOf) SetParams(v CreateRouteRequestProcessorsInnerAnyOfParams) {
	o.Params = v
}

func (o CreateRouteRequestProcessorsInnerAnyOf) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o CreateRouteRequestProcessorsInnerAnyOf) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["id"] = o.Id
	toSerialize["name"] = o.Name
	if !IsNil(o.Description) {
		toSerialize["description"] = o.Description
	}
	if !IsNil(o.Example) {
		toSerialize["example"] = o.Example
	}
	if !IsNil(o.Tags) {
		toSerialize["tags"] = o.Tags
	}
	if !IsNil(o.UpdatedAt) {
		toSerialize["updatedAt"] = o.UpdatedAt
	}
	toSerialize["type"] = o.Type
	toSerialize["params"] = o.Params
	return toSerialize, nil
}

func (o *CreateRouteRequestProcessorsInnerAnyOf) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"id",
		"name",
		"type",
		"params",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varCreateRouteRequestProcessorsInnerAnyOf := _CreateRouteRequestProcessorsInnerAnyOf{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varCreateRouteRequestProcessorsInnerAnyOf)

	if err != nil {
		return err
	}

	*o = CreateRouteRequestProcessorsInnerAnyOf(varCreateRouteRequestProcessorsInnerAnyOf)

	return err
}

type NullableCreateRouteRequestProcessorsInnerAnyOf struct {
	value *CreateRouteRequestProcessorsInnerAnyOf
	isSet bool
}

func (v NullableCreateRouteRequestProcessorsInnerAnyOf) Get() *CreateRouteRequestProcessorsInnerAnyOf {
	return v.value
}

func (v *NullableCreateRouteRequestProcessorsInnerAnyOf) Set(val *CreateRouteRequestProcessorsInnerAnyOf) {
	v.value = val
	v.isSet = true
}

func (v NullableCreateRouteRequestProcessorsInnerAnyOf) IsSet() bool {
	return v.isSet
}

func (v *NullableCreateRouteRequestProcessorsInnerAnyOf) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCreateRouteRequestProcessorsInnerAnyOf(val *CreateRouteRequestProcessorsInnerAnyOf) *NullableCreateRouteRequestProcessorsInnerAnyOf {
	return &NullableCreateRouteRequestProcessorsInnerAnyOf{value: val, isSet: true}
}

func (v NullableCreateRouteRequestProcessorsInnerAnyOf) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCreateRouteRequestProcessorsInnerAnyOf) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


