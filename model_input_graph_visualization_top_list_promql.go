/*
Tsuga Public API

Public HTTP API for Tsuga customers and customer-operated tools. Use these endpoints to query observability data, manage customer-owned Tsuga resources, and retrieve documentation or API-reference content. Public API requests authenticate with Bearer tokens such as operation keys. See [API reference](/documentation/api).

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package tsuga

import (
	"encoding/json"
	"fmt"
)

// checks if the InputGraphVisualizationTopListPromql type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &InputGraphVisualizationTopListPromql{}

// InputGraphVisualizationTopListPromql struct for InputGraphVisualizationTopListPromql
type InputGraphVisualizationTopListPromql struct {
	// Displays PromQL metrics query-based aggregation as a ranked top list
	Type string `json:"type"`
	// PromQL expressions configured for this widget.
	Queries    []string                                         `json:"queries"`
	Aliases    *InputGraphVisualizationTimeseriesPromqlAliases  `json:"aliases,omitempty"`
	Normalizer *Normalizer1                                     `json:"normalizer,omitempty"`
	Precision  *GraphVisualizationQueryValueConnectionPrecision `json:"precision,omitempty"`
	// Conditional formatting rules applied to the displayed value
	Conditions           []ConditionalFormatting `json:"conditions,omitempty"`
	AdditionalProperties map[string]interface{}
}

type _InputGraphVisualizationTopListPromql InputGraphVisualizationTopListPromql

// NewInputGraphVisualizationTopListPromql instantiates a new InputGraphVisualizationTopListPromql object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewInputGraphVisualizationTopListPromql(type_ string, queries []string) *InputGraphVisualizationTopListPromql {
	this := InputGraphVisualizationTopListPromql{}
	this.Type = type_
	this.Queries = queries
	return &this
}

// NewInputGraphVisualizationTopListPromqlWithDefaults instantiates a new InputGraphVisualizationTopListPromql object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewInputGraphVisualizationTopListPromqlWithDefaults() *InputGraphVisualizationTopListPromql {
	this := InputGraphVisualizationTopListPromql{}
	return &this
}

// GetType returns the Type field value
func (o *InputGraphVisualizationTopListPromql) GetType() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Type
}

// GetTypeOk returns a tuple with the Type field value
// and a boolean to check if the value has been set.
func (o *InputGraphVisualizationTopListPromql) GetTypeOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Type, true
}

// SetType sets field value
func (o *InputGraphVisualizationTopListPromql) SetType(v string) {
	o.Type = v
}

// GetQueries returns the Queries field value
func (o *InputGraphVisualizationTopListPromql) GetQueries() []string {
	if o == nil {
		var ret []string
		return ret
	}

	return o.Queries
}

// GetQueriesOk returns a tuple with the Queries field value
// and a boolean to check if the value has been set.
func (o *InputGraphVisualizationTopListPromql) GetQueriesOk() ([]string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Queries, true
}

// SetQueries sets field value
func (o *InputGraphVisualizationTopListPromql) SetQueries(v []string) {
	o.Queries = v
}

// GetAliases returns the Aliases field value if set, zero value otherwise.
func (o *InputGraphVisualizationTopListPromql) GetAliases() InputGraphVisualizationTimeseriesPromqlAliases {
	if o == nil || IsNil(o.Aliases) {
		var ret InputGraphVisualizationTimeseriesPromqlAliases
		return ret
	}
	return *o.Aliases
}

// GetAliasesOk returns a tuple with the Aliases field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InputGraphVisualizationTopListPromql) GetAliasesOk() (*InputGraphVisualizationTimeseriesPromqlAliases, bool) {
	if o == nil || IsNil(o.Aliases) {
		return nil, false
	}
	return o.Aliases, true
}

// HasAliases returns a boolean if a field has been set.
func (o *InputGraphVisualizationTopListPromql) HasAliases() bool {
	if o != nil && !IsNil(o.Aliases) {
		return true
	}

	return false
}

// SetAliases gets a reference to the given InputGraphVisualizationTimeseriesPromqlAliases and assigns it to the Aliases field.
func (o *InputGraphVisualizationTopListPromql) SetAliases(v InputGraphVisualizationTimeseriesPromqlAliases) {
	o.Aliases = &v
}

// GetNormalizer returns the Normalizer field value if set, zero value otherwise.
func (o *InputGraphVisualizationTopListPromql) GetNormalizer() Normalizer1 {
	if o == nil || IsNil(o.Normalizer) {
		var ret Normalizer1
		return ret
	}
	return *o.Normalizer
}

// GetNormalizerOk returns a tuple with the Normalizer field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InputGraphVisualizationTopListPromql) GetNormalizerOk() (*Normalizer1, bool) {
	if o == nil || IsNil(o.Normalizer) {
		return nil, false
	}
	return o.Normalizer, true
}

// HasNormalizer returns a boolean if a field has been set.
func (o *InputGraphVisualizationTopListPromql) HasNormalizer() bool {
	if o != nil && !IsNil(o.Normalizer) {
		return true
	}

	return false
}

// SetNormalizer gets a reference to the given Normalizer1 and assigns it to the Normalizer field.
func (o *InputGraphVisualizationTopListPromql) SetNormalizer(v Normalizer1) {
	o.Normalizer = &v
}

// GetPrecision returns the Precision field value if set, zero value otherwise.
func (o *InputGraphVisualizationTopListPromql) GetPrecision() GraphVisualizationQueryValueConnectionPrecision {
	if o == nil || IsNil(o.Precision) {
		var ret GraphVisualizationQueryValueConnectionPrecision
		return ret
	}
	return *o.Precision
}

// GetPrecisionOk returns a tuple with the Precision field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InputGraphVisualizationTopListPromql) GetPrecisionOk() (*GraphVisualizationQueryValueConnectionPrecision, bool) {
	if o == nil || IsNil(o.Precision) {
		return nil, false
	}
	return o.Precision, true
}

// HasPrecision returns a boolean if a field has been set.
func (o *InputGraphVisualizationTopListPromql) HasPrecision() bool {
	if o != nil && !IsNil(o.Precision) {
		return true
	}

	return false
}

// SetPrecision gets a reference to the given GraphVisualizationQueryValueConnectionPrecision and assigns it to the Precision field.
func (o *InputGraphVisualizationTopListPromql) SetPrecision(v GraphVisualizationQueryValueConnectionPrecision) {
	o.Precision = &v
}

// GetConditions returns the Conditions field value if set, zero value otherwise.
func (o *InputGraphVisualizationTopListPromql) GetConditions() []ConditionalFormatting {
	if o == nil || IsNil(o.Conditions) {
		var ret []ConditionalFormatting
		return ret
	}
	return o.Conditions
}

// GetConditionsOk returns a tuple with the Conditions field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *InputGraphVisualizationTopListPromql) GetConditionsOk() ([]ConditionalFormatting, bool) {
	if o == nil || IsNil(o.Conditions) {
		return nil, false
	}
	return o.Conditions, true
}

// HasConditions returns a boolean if a field has been set.
func (o *InputGraphVisualizationTopListPromql) HasConditions() bool {
	if o != nil && !IsNil(o.Conditions) {
		return true
	}

	return false
}

// SetConditions gets a reference to the given []ConditionalFormatting and assigns it to the Conditions field.
func (o *InputGraphVisualizationTopListPromql) SetConditions(v []ConditionalFormatting) {
	o.Conditions = v
}

func (o InputGraphVisualizationTopListPromql) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o InputGraphVisualizationTopListPromql) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["type"] = o.Type
	toSerialize["queries"] = o.Queries
	if !IsNil(o.Aliases) {
		toSerialize["aliases"] = o.Aliases
	}
	if !IsNil(o.Normalizer) {
		toSerialize["normalizer"] = o.Normalizer
	}
	if !IsNil(o.Precision) {
		toSerialize["precision"] = o.Precision
	}
	if !IsNil(o.Conditions) {
		toSerialize["conditions"] = o.Conditions
	}

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

func (o *InputGraphVisualizationTopListPromql) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"type",
		"queries",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err
	}

	for _, requiredProperty := range requiredProperties {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varInputGraphVisualizationTopListPromql := _InputGraphVisualizationTopListPromql{}

	err = json.Unmarshal(data, &varInputGraphVisualizationTopListPromql)

	if err != nil {
		return err
	}

	*o = InputGraphVisualizationTopListPromql(varInputGraphVisualizationTopListPromql)

	additionalProperties := make(map[string]interface{})

	if err = json.Unmarshal(data, &additionalProperties); err == nil {
		delete(additionalProperties, "type")
		delete(additionalProperties, "queries")
		delete(additionalProperties, "aliases")
		delete(additionalProperties, "normalizer")
		delete(additionalProperties, "precision")
		delete(additionalProperties, "conditions")
		o.AdditionalProperties = additionalProperties
	}

	return err
}

type NullableInputGraphVisualizationTopListPromql struct {
	value *InputGraphVisualizationTopListPromql
	isSet bool
}

func (v NullableInputGraphVisualizationTopListPromql) Get() *InputGraphVisualizationTopListPromql {
	return v.value
}

func (v *NullableInputGraphVisualizationTopListPromql) Set(val *InputGraphVisualizationTopListPromql) {
	v.value = val
	v.isSet = true
}

func (v NullableInputGraphVisualizationTopListPromql) IsSet() bool {
	return v.isSet
}

func (v *NullableInputGraphVisualizationTopListPromql) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableInputGraphVisualizationTopListPromql(val *InputGraphVisualizationTopListPromql) *NullableInputGraphVisualizationTopListPromql {
	return &NullableInputGraphVisualizationTopListPromql{value: val, isSet: true}
}

func (v NullableInputGraphVisualizationTopListPromql) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableInputGraphVisualizationTopListPromql) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
